...
/**
 * This class represents a Blur Filter command that can be performed on an object
 * of an image model.
 */
public class BlurFilter extends AbstractFilterCommand {
  /**
   * Constructs a Blur Filter command using a given output, map of image models, and scanner.
   *
   ...
   */
  public BlurFilter(Appendable output, ImageModelMap imageModelMap,
                       Scanner scanner) throws IllegalArgumentException {
    super(output, imageModelMap, scanner);
  }

  /**
   * Creates a new image model by performing a Blur Filter method on a given image model.
   *
  ...
  protected ImageModel newModel(ImageModel oldModel) {
    return filter(oldModel, Filter.BLUR);
  }
}