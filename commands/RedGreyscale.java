...
/**
 * This class represents a Red Greyscale command that can be performed on an object
 * of an image model.
 */
public class RedGreyscale extends AbstractTransformationCommand {
  /**
   * Constructs a Red Greyscale command using a given output, map of image models, and scanner.
   *
   ...
   */
  public RedGreyscale(Appendable output, ImageModelMap imageModelMap,
                      Scanner scanner) throws IllegalArgumentException {
    super(output, imageModelMap, scanner);
  }

  /**
   * Creates a new image model by performing a Red Greyscale method on a given image model.
   *
  ...
  protected ImageModel newModel(ImageModel oldModel) {
    return transform(oldModel, Transformation.RED);
  }
}
