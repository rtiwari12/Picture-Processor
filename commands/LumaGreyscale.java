...
/**
 * This class represents a Luma Greyscale command that can be performed on an object
 * of an image model.
 */
public class LumaGreyscale extends AbstractTransformationCommand {
  /**
   * Constructs a Luma Greyscale command using a given output, map of image models, and scanner.
   *
   ...
   */
  public LumaGreyscale(Appendable output, ImageModelMap imageModelMap,
                       Scanner scanner) throws IllegalArgumentException {
    super(output, imageModelMap, scanner);
  }

  /**
   * Creates a new image model by performing a Luma Greyscale method on a given image model.
   *
  ...
  protected ImageModel newModel(ImageModel oldModel) {
    return transform(oldModel, Transformation.LUMA);
  }
}